...
### Branding 與 Verification checklist

#### Branding

- App name：`Tomato Planner`
- User support email：使用可接收使用者問題的專案信箱。
- App home page：正式部署的 HTTPS 首頁。
- Privacy Policy URL：正式 origin 下的 `/privacy`。
- Terms of Service URL：正式 origin 下的 `/terms`。
- Developer contact email：填入會實際維護的聯絡信箱。

#### Audience

- 公開 App 的 user type 選擇 `External`。
- 測試期間只有 Test users 可使用；準備公開時再切換 `Production`。
- Production 但未完成要求的 verification 前，仍可能出現警告與 100 user cap。

#### Data Access

- 僅保留 `https://www.googleapis.com/auth/drive.appdata`。
- 僅保留 `https://www.googleapis.com/auth/calendar.readonly`。
- 不要加入完整 `drive` scope 或可寫入的完整 Calendar scope。

#### Verification Center

1. 先完成並發布 Branding，確認公開頁面 URL 可不登入直接開啟。
2. 再準備 Data Access verification。
3. 說明 `calendar.readonly` 只用於在排程畫面顯示行程，不會寫入。
4. 說明 `drive.appdata` 只用於 App 專用備份，不讀取一般 Drive 檔案。
5. 使用 [OAUTH_VERIFICATION.md](./OAUTH_VERIFICATION.md) 的中英文說明準備表單與演示。
...
根頁面，因此 `/tasks`、`/schedule`、`/pomodoro`、`/settings`、`/privacy` 與
`/terms` 重新整理後應能載入。
...
- [ ] 首頁與所有導覽頁面可開啟。
- [ ] `/privacy` 與 `/terms` 可不授權 Google 直接開啟與重新整理。
